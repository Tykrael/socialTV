requirejs.config( {
	baseUrl :  URL_STATIC_RES + '/js/widgetSocial/',
	paths:{
		text: 			URL_STATIC_RES + '/js/widgetSocial/libs/require/text',
		
		//config:			URL_STATIC_RES + '/js/widgets/ftv.widgetSocial/js/widgetConfig',
		jquery:			URL_STATIC_RES + '/js/widgetSocial/libs/jquery-1.7.2',
		underscore:		URL_STATIC_RES + '/js/widgetSocial/libs/underscore/lo-dash-1.0.0.min',
		massrel:		URL_STATIC_RES + '/js/widgetSocial/libs/massrel.stream',
		iscroll:		URL_STATIC_RES + '/js/widgetSocial/libs/iscroll',
		spinner:		URL_STATIC_RES + '/js/widgetSocial/libs/spinner',
		require:		URL_STATIC_RES + '/js/widgetSocial/libs/require-2.1.2.min'
	},
	...
	*/
			...
			this.newPosts = function(data,index){
				...
				$.each(data,function(k,val){
					...
					//console.log($('#widgetContainer'+index+' .itemList .live[rel="post-'+post.messageId+'"]').length);
					if(!$('#widgetContainer'+index+' .itemList .live[rel="post-'+post.messageId+'"]').length)
						listPosts.push(post)
				})
